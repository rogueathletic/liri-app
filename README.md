...
<ul>
  ...
  <li>case/break - although these two processes are different they work really well in allowin gthe developer to structure many values to form a single function to formulate all three different API calls. <ul><li>Case: listens for the submitted command<li>Break sends the code to the next line to refference the ===<li>err - if all else fails err is enacted notifying the user the er was an error</li></ul></li><li>split (",") - tells the code if i user a command like  ```node liri.js concert-this Ariana Grande``` the return will be submitted as ```node liri.js "ariana grande"</li>
</ul>
